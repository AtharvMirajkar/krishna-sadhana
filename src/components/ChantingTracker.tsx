...
import { getUserId, formatDate, getStartOfWeek, getStartOfMonth } from '../lib/utils';
import type { Database } from '../lib/database.types';

type Mantra = Database['public']['Tables']['mantras']['Row'];
type ChantingRecord = Database['public']['Tables']['chanting_records']['Row'];
...
export function ChantingTracker() {
  ...
  async function loadStats() {
    try {
      ...
      const monthStart = formatDate(getStartOfMonth(today));

      const [mantrasRes, recordsRes] = await Promise.all([
        supabase.from('mantras').select('*').order('created_at'),
        supabase
          .from('chanting_records')
          .select('*')
          .eq('user_id', userId)
          .gte('chant_date', monthStart)
      ]);

      if (mantrasRes.error) throw mantrasRes.error;
      ...
      const mantraStatsMap: Record<string, MantraStats> = {};

      mantras.forEach(mantra => {
        mantraStatsMap[mantra.id] = {
          mantra,
          today: 0,
          week: 0,
          month: 0
      ...
      records.forEach(record => {
        ...
        if (mantraStatsMap[record.mantra_id]) {
          mantraStatsMap[record.mantra_id].month += count;

          if (recordDate >= weekStart) {
            mantraStatsMap[record.mantra_id].week += count;
          }

          if (recordDate === todayStr) {
            mantraStatsMap[record.mantra_id].today += count;
          }
        }
      });
      ...
      setMantraStats(Object.values(mantraStatsMap));
    } catch (error) {
