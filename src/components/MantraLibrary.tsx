...
export function MantraLibrary() {
  ...
  const [chantingRecords, setChantingRecords] = useState<Record<string, number>>({});
  const [incrementingMantra, setIncrementingMantra] = useState<string | null>(null);
  ...
  async function loadTodayChanting() {
    try {
      ...
      console.error('Error loading chanting records:', error);
    }
  }

  async function incrementChant(mantraId: string) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({
          ...
          onConflict: 'mantra_id,user_id,chant_date'
        });

      if (error) throw error;
    } catch (error) {
      console.error('Error incrementing chant:', error);
      setChantingRecords(prev => ({
        ...prev,
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                  ...
                  <div className="flex flex-col items-center gap-4 min-w-[140px]">
                    <div className="bg-gradient-to-br from-amber-400 to-orange-500 rounded-2xl p-6 text-center shadow-lg">
                      ...
                      <div className="text-white text-sm font-medium">Today's Count</div>
                    </div>

                    <button
                      onClick={() => incrementChant(mantra.id)}
                      disabled={incrementingMantra === mantra.id}
                      className={`w-full px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform transition-all duration-200 flex items-center justify-center gap-2 ${
